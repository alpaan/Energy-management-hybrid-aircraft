...
% Function: Interpolate the electric motor loss coefficients from data.
% For a second order fit, the loss map is given by:
%
...
% solving a least square problem for the second order fit (resp. 
% constrained least square for the first order fit) The alpha coefficients 
% are then interpolated for the input estimated velocity vector w_est. 
% with a spline interpolation method.
%
% Input: 
...
mech_power = electric_motor.P_em;  % output (mech.) power per shaft (MW)
elec_power = electric_motor.P_c;  % input (elec.) power per shaft (MW)
...
for i=1:N  % loop over speed data
    ...
    if (n == 1)
        ...
        % Find coefficients using constrained least square to avoid
        % alpha0 < 0 (which would yield Pc < 0 when Pem = 0. 
        % Optim. problem: min_{beta} 0.5 || X*beta - Y|| s.t. beta0 >=0
        % Analytical solution obtained using KKT condition 
        M = inv(X'*X);
        lambda = M*X'*Y/M(1,1);  % Lagrange multiplier
        beta = inv(X'*X)*(X'*Y - [lambda(1); 0]);  % constrained LS fit
...
for i=1:n+1
    ...
    alpha(:, i) = interp1(w, A(:,i), w_est, 'spline'); 
end 
